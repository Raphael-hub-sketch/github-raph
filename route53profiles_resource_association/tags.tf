  ...
  common_tags = merge(
    {
      ...
      ManagedBy   = "terraform"
      Component   = "dns-profiles"
      Service     = "route53-profiles"
    },
    {
      # Имитация расширенного набора тегов для FinOps
      BillingCode        = "PROFILES-${upper(var.environment)}-001"
      ...
      VpcAssociations    = tostring(local.profiles_usage.vpc_associations)
    }
  )
}